...
导入器也会识别同一行中的未转义标记，方便直接粘贴 `[PROMPT] Character 正面提示词` 或整行压缩的内容。如果提示词本身需要包含标记文本，在标记前添加反斜杠。例如 `\[CHARACTER]` 导入后会成为字面文本 `[CHARACTER]`。如果提示词的一行本身以反斜杠开头，导出时也在行首再添加一个反斜杠，以保证导入后能够完整还原。
...
- 导出时必须按照“保留标记与转义”的规则处理提示词正文，使导出的文本能够再次导入并还原。
