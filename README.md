...
NovelAI 空闲时，Generate 仍然是原来的 Generate，行为完全不变。正在生成或队列已有任务时，按钮会变成紫蓝色；此时点击会把当前配置排到队尾。

排队时会**冻结**当时的完整配置：Prompt、模型、参数，以及 Image2Image、Vibe Transfer 和 Precise Reference 中使用的图片。之后继续修改页面，不会污染已经记录的任务。

任务严格串行执行，不并发请求。遇到 429、服务端错误或网络中断时会自动重试；包括 Seed 在内的请求参数都按入队时的内容保留，重试也不会擅自改写。

点击入队按钮时，工具会立即调用 NovelAI 原始生成流程校验参数，同时截住其生成请求，不向服务器发送。固定 Seed 与上次生成参数完全相同时，NovelAI 会当场显示自己的重复参数提示；工具不会自行仿造判断，也不会保留被拦截的任务。只有原始流程确实产生了生成请求，配置才会正式进入队列。

队列启用、队列禁用、删除、清空和重新入队都包含在面板里。
...
使用固定 Seed 时，如果列表中出现相邻且完全相同的替换单位，脚本会在启动前提醒；可以确认继续，也可以取消并先修改列表。
